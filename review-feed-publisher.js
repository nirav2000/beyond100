...
function mount(){
  ...
  el.querySelector('#copyStaticReview').onclick=async()=>{const u=feedMeta()?.rawUrl;if(!u)return;try{await navigator.clipboard.writeText(u);setStatus('ChatGPT JSON link copied.')}catch{}};
...
function renderPanel(){const raw=document.querySelector('#staticReviewRawUrl'),page=document.querySelector('#staticReviewPageUrl');if(!raw||!page)return;const meta=feedMeta();raw.value=meta?.rawUrl||'';page.value=meta?.url||'';setStatus(meta?.lastPublishedAt?`Last published ${new Date(meta.lastPublishedAt).toLocaleString('en-GB')}. Treat either URL like a password.`:'')}
