...
async function startCloud(){const S=await sdk();await S.auth.authStateReady();S.Auth.onAuthStateChanged(S.auth,async user=>{cloudReady=!!user&&user.uid===CLOUD.ownerUid;if(!cloudReady){setCloud('Saved locally · sign in to Firebase to sync','local');return}try{setCloud('Syncing learning evidence…','syncing');window.FirebaseUsageMonitor?.read(1,'learning-evidence-read','beyond100');const snap=await S.F.getDoc(ref(S));if(snap.exists()&&snap.data()?.payload){state=merge(state,snap.data().payload);localStorage.setItem(EVIDENCE_KEY,JSON.stringify(state));renderAll()}await syncCloud()}catch{setCloud('Saved locally · evidence sync failed','error')}})}
function scheduleCloudSync(){clearTimeout(cloudSyncTimer);if(cloudReady)cloudSyncTimer=setTimeout(()=>syncCloud().catch(()=>setCloud('Saved locally · evidence sync failed','error')),2500)}
async function syncCloud(){if(!cloudReady)return;const S=await sdk(),u=S.auth.currentUser;if(!u||u.uid!==CLOUD.ownerUid)return;setCloud('Syncing learning evidence…','syncing');window.FirebaseUsageMonitor?.write(1,'learning-evidence-write','beyond100');await S.F.setDoc(ref(S),{app:'beyond100',kind:'learning-evidence',schema:state.schema,updatedAt:state.updatedAt||now(),payload:state},{merge:true});setCloud('Learning evidence synced to Firebase','connected')}
