(()=>{
  ...
  const CANDIDATES='[data-note-anchor],section,article,details,[class*="card"],[class*="panel"],[class*="block"]';
  const IGNORE='[data-note-ignore="true"],.notes-dialog,.annotation-banner,.selection-note-button,nav,footer,script,style';
  ...
  function annotate(root=document){
    const main=root.matches?.(ROOT_SELECTOR)?root:root.querySelector?.(ROOT_SELECTOR)||document.querySelector(ROOT_SELECTOR);
    if(!main)return;
    [...main.querySelectorAll(CANDIDATES)].forEach((el,index)=>{
      if(!useful(el))return;
