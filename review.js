...
function feedId(){
  ...
  return /^[A-Za-z0-9_-]{40,120}$/.test(raw)?raw:'';
}
function rawUrl(id){
  const project=CLOUD?.firebase?.projectId||'kk-syllabus';
  const key=CLOUD?.firebase?.apiKey||'';
...
function render(feed,id){
  const notes=Array.isArray(feed.notes)?feed.notes:[];
  ...
  list.innerHTML=notes.length?notes.map((n,i)=>`<article class="review-note">
    ...
    <div class="meta"><span>${esc(n.anchorId||'page:general')}</span>${n.section?`<span>${esc(n.section)}</span>`:''}${n.topic?`<span>${esc(n.topic)}</span>`:''}${n.version?`<span>v${esc(n.version)}</span>`:''}<span>${esc(n.id||'')}</span></div>
  </article>`).join(''):'<article class="review-note"><h2>No pending review notes</h2><p class="context">The feed is working; there are currently no notes marked for ChatGPT review.</p></article>';
  setStatus(`Loaded ${notes.length} pending review note${notes.length===1?'':'s'} from Firebase.`,'good');
}
...
async function loadFeed(){
  ...
  try{
    ...
    if(feed.app!=='beyond100')throw new Error('This capability does not point to a Beyond 100 review feed.');
    render(feed,id);
  }catch(e){
    console.error(e);setStatus(e?.message||'Could not load the review feed.','error');summary.hidden=true;list.innerHTML='';
  }
}

async function copy(value,button){
  try{await navigator.clipboard.writeText(value);const old=button.textContent;button.textContent='Copied ✓';setTimeout(()=>button.textContent=old,1200)}catch{}
}
document.querySelector('#refreshFeed').onclick=loadFeed;
document.querySelector('#copyPageLink').onclick=e=>copy(location.href,e.currentTarget);
document.querySelector('#copyRawLink').onclick=e=>{const id=feedId();if(id)copy(rawUrl(id),e.currentTarget)};
loadFeed();
